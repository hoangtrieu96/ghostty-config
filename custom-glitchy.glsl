...
// Same timing/amplitude as the original glow-rgbsplit-twitchy.glsl —
// ABBERATION_FACTOR below is what actually scales down the visible split.
...
// Reduced from 0.05 to 0.02 (max shift ~2.7px instead of ~6.8px) so the
// RGB split is calmer and text stays readable, while still animating.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float amount = offsetFunction(iTime) * GLITCH_INTENSITY;
    ...
    if (source.x > DIM_CUTOFF) {
        ...
        dest.xyz += glow.xyz * GLOW_INTENSITY;
    }
